...
/** Human labels used by the audit log and change history. */
export const FIELD_LABELS: Record<string, string> = {
  ...
  handloom_culture_id: 'Handloom culture',
  region: 'Region',
  state: 'State',
  gender: 'Gender',
  ...
  blouse_colour: 'Blouse colour',
  zari: 'Zari',
  transparency: 'Transparency',
  fall_pico_status: 'Fall / pico status',
  fit: 'Fit',
  neckline: 'Neckline',
  sleeve: 'Sleeve',
  ...
  pockets: 'Pockets',
  stretch: 'Stretch',
  artisan_name: 'Artisan / weaver',
  artisan_story: 'Weaver story',
  craft_story: 'Craft story',
  historical_context: 'Historical context',
  cultural_significance: 'Cultural significance',
  yarn: 'Yarn',
  zari_type: 'Zari type',
  dyeing_method: 'Dyeing method',
  embroidery: 'Embroidery',
  special_techniques: 'Special techniques',
  certification: 'Certification',
  origin: 'Origin',
  size_guide_id: 'Size guide',
  measurements: 'Measurements',
  short_description: 'Short description',
  full_description: 'Full description',
  story: 'The story',
  about_the_weave: 'About the weave',
  about_the_artisan: 'About the artisan',
  details: 'Details',
  care: 'Care',
  shipping_notes: 'Shipping / packaging notes',
  price: 'Price',
  ...
  handle_locked: 'Handle locked (manual override)',
...
/** Columns a normal product edit may touch (SKU and audit columns excluded). */
...
export function listProducts(filters: ProductFilters = {}): { rows: ProductRow[]; total: number } {
  ...
  if (filters.missingPhotography) {
    where.push('p.photography_required > 0 AND p.photography_complete < p.photography_required');
  }
  if (filters.awaitingApproval) {
    where.push(`p.status IN ('CONTENT_REVIEW','INTERNAL_REVIEW','FOUNDER_APPROVAL','CHANGES_REQUESTED')`);
  }
  ...
  const total = get<{ c: number }>(`SELECT COUNT(*) AS c ${from} ${clause}`, params)?.c ?? 0;
  ...
  const rows = all<ProductRow>(
    `${PRODUCT_SELECT} ${clause} ORDER BY ${sortColumn} ${direction}, p.sku ASC LIMIT ? OFFSET ?`,
...
export function bundleFor(product: ProductRow): ProductBundle {
  const variants = all<VariantRow>('SELECT * FROM variant WHERE product_id = ? ORDER BY position, sku', [product.id]);
  ...
  const slots = all<{ id: string; key: string; label: string; is_required: number }>(
...
/**
 * Bumps a candidate SKU until no existing product claims it. Needed because the
 * sequence may be scoped to a culture while SKUs are globally unique.
 */
...
export function createProduct(input: CreateProductInput, userId: string): ProductRow {
  ...
  const category = input.categoryId
    ? get<{ name: string; sku_segment: string | null; template_key: string }>(
        'SELECT name, sku_segment, template_key FROM category WHERE id = ?',
        [input.categoryId],
      )
    : null;
  ...
  if (!sku) {
    const prefix = buildSkuPrefix(pattern, {
      type: category?.sku_segment,
      culture: culture?.code,
      category: category?.sku_segment,
    });
    ...
    sku = allocateFreeSku({
      candidate: generateSku({ pattern, padding, prefix, existingSkus: scoped }),
      existingSkus,
      padding,
    });
  }
  ...
  transaction(() => {
    run(
      ...
      [
        ...
        name,
        name ? 'NAME_PROPOSED' : 'NOT_NAMED',
        input.categoryId ?? null,
        input.subcategoryId ?? null,
        input.handloomCultureId ?? null,
        input.productTypeLabel ?? category?.name ?? null,
        input.assigneeId ?? null,
        input.price ?? null,
        handle,
        userId,
        userId,
        getSetting('units.currency') || 'INR',
        stamp,
        stamp,
      ],
    );
    // Every product starts with a default variant so pricing and Shopify
    // exports work before the team adds real size/colour variants.
    run(
      `INSERT INTO variant
    ...
    logAudit({
      ...
      action: 'CREATE',
      userId,
      changes: [{ field: 'sku', label: 'SKU', oldValue: null, newValue: sku }],
    });
  });
...
export function updateProduct(
  ...
  userId: string,
  opts: { action?: string; reason?: string } = {},
): UpdateResult {
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    changes.push({
      ...
      oldValue: (before as unknown as Record<string, unknown>)[key] ?? null,
      newValue: stored ?? null,
    });
  }

  // Keep the name workflow in sync with the name field.
  if (patch.name !== undefined && patch.name_status === undefined) {
    ...
    if (nextStatus !== before.name_status) {
      sets.push('name_status = ?');
      params.push(nextStatus);
      changes.push({ field: 'name_status', label: 'Name status', oldValue: before.name_status, newValue: nextStatus });
    }
    // Regenerate the handle unless it was manually locked (spec §31) or the
    // caller supplied one explicitly in this same patch.
    ...
    if (nextName && !lockedNow && patch.handle === undefined) {
      const handles = all<{ handle: string }>(
        'SELECT handle FROM product WHERE handle IS NOT NULL AND id != ?',
        [productId],
      ).map((r) => r.handle);
      const nextHandle = uniqueHandle(nextName, handles, before.sku);
      if (nextHandle !== before.handle) {
        sets.push('handle = ?');
        params.push(nextHandle);
  ...
  if (sets.length > 0) {
    const stamp = nowIso();
    run(
      `UPDATE product SET ${sets.join(', ')}, modified_by_id = ?, updated_at = ? WHERE id = ?`,
      [...params, userId, stamp, productId],
    );
  }
...
export function setAttributeValues(
  productId: string,
  values: Array<{ fieldId: string; value: string; isMissing?: boolean; note?: string | null }>,
  userId: string,
): void {
  const before = getProduct(productId);
  if (!before) throw new Error('Product not found.');
  const existing = all<{ field_id: string; value: string; is_missing: number; label: string }>(
    `SELECT v.field_id, v.value, v.is_missing, f.label
     FROM product_attribute_value v JOIN attribute_field f ON f.id = v.field_id
     WHERE v.product_id = ?`,
    [productId],
  );
  ...
  for (const entry of values) {
    ...
    const nextMissing = entry.isMissing ? 1 : 0;
    if (previous && previous.value === nextValue && previous.is_missing === nextMissing) continue;
    run(
      `INSERT INTO product_attribute_value (id, product_id, field_id, value, is_missing, note)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(product_id, field_id) DO UPDATE SET value = excluded.value, is_missing = excluded.is_missing, note = excluded.note`,
      [cuid(), productId, entry.fieldId, nextValue, nextMissing, entry.note ?? null],
    );
    changes.push({
      field: `attr:${entry.fieldId}`,
      label,
      oldValue: previous ? (previous.is_missing ? 'MISSING — INFORMATION REQUIRED' : previous.value) : null,
      newValue: nextMissing ? 'MISSING — INFORMATION REQUIRED' : nextValue,
    });
  }
...
export function snapshotProduct(productId: string, userId: string | null, reason?: string): void {
  ...
  for (const key of [
    'name', 'name_status', 'status', 'price', 'compare_at_price', 'cost_price', 'short_description',
    'full_description', 'story', 'about_the_weave', 'care', 'measurements', 'category_id',
    'handloom_culture_id', 'seo_title', 'seo_description', 'handle', 'shopify_status', 'tags',
  ]) {
    tracked[key] = (product as unknown as Record<string, unknown>)[key];
  }
  run(
    `INSERT INTO product_snapshot (id, product_id, version, reason, data, user_id, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
export function setProductStatus(
  ...
  userId: string,
  opts: { comment?: string; decision?: string; checklist?: Record<string, boolean> } = {},
): void {
  const before = getProduct(productId);
  if (!before) throw new Error('Product not found.');
  ...
  if (opts.decision) {
    run(
      `INSERT INTO product_review (id, product_id, user_id, decision, checklist, comment, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
  ...
  run('UPDATE product SET status = ?, modified_by_id = ?, updated_at = ? WHERE id = ?', [
    status,
    userId,
    nowIso(),
    productId,
  ...
  logAudit({
    ...
    entityId: productId,
    entityLabel: before.sku,
    action: opts.decision ?? 'STATUS_CHANGE',
    changes: [{ field: 'status', label: 'Status', oldValue: before.status, newValue: status }],
    meta: opts.comment ? { comment: opts.comment } : null,
    userId,
  });
}
...
export function archiveProduct(productId: string, archived: boolean, userId: string): void {
  const before = getProduct(productId);
  if (!before) return;
  run('UPDATE product SET is_archived = ?, status = ?, modified_by_id = ?, updated_at = ? WHERE id = ?', [
    archived ? 1 : 0,
  ...
  logAudit({
    ...
    changes: [{ field: 'is_archived', label: 'Archived', oldValue: before.is_archived, newValue: archived ? 1 : 0 }],
    userId,
  });
}
...
export function deleteProduct(productId: string, userId: string): void {
  ...
  logAudit({
    ...
    entityLabel: before.sku,
    action: 'DELETE',
    changes: [{ field: 'sku', label: 'SKU', oldValue: before.sku, newValue: null }],
    userId,
  });
}
...
export function recomputeProduct(
...
): { completeness: number; readiness: string } {
  ...
  const otherHandles = all<{ handle: string }>(
    'SELECT handle FROM product WHERE handle IS NOT NULL AND id != ?',
    [productId],
  ).map((r) => r.handle.toLowerCase());
  const otherSkus = all<{ sku: string }>('SELECT sku FROM product WHERE id != ?', [productId]).map((r) => r.sku.toUpperCase());
  ...
  const readiness = assessReadiness(bundle, {
    usedHandles: new Set(otherHandles),
    usedSkus: new Set(otherSkus),
    imagesPubliclyAccessible: bundle.images.length > 0 ? imagesPublic : undefined,
  });

  run(
    `UPDATE product SET completeness_score = ?, readiness_state = ?, readiness_score = ?,
       readiness_issues = ?, photography_complete = ?, photography_required = ?
     WHERE id = ?`,
    [
      completeness.score,
      readiness.state,
      readiness.score,
      json(readiness.issues),
      photo.complete,
      photo.required,
      productId,
    ],
...
export function setProductCollections(productId: string, collectionIds: string[], userId: string): void {
  const before = getProduct(productId);
  if (!before) return;
  const current = all<{ collection_id: string; name: string }>(
    `SELECT pc.collection_id, c.name FROM product_collection pc
     JOIN collection c ON c.id = pc.collection_id WHERE pc.product_id = ?`,
    [productId],
  );
  ...
  for (const id of currentIds) {
    ...
    run('DELETE FROM product_collection WHERE product_id = ? AND collection_id = ?', [productId, id]);
  }
  for (const id of nextIds) {
    if (currentIds.has(id)) continue;
    run(
      `INSERT INTO product_collection (id, product_id, collection_id, added_by_id, created_at)
       VALUES (?, ?, ?, ?, ?)`,
      [cuid(), productId, id, userId, nowIso()],
    );
  }
  const names = all<{ id: string; name: string }>('SELECT id, name FROM collection').reduce<Record<string, string>>(
    (acc, row) => ({ ...acc, [row.id]: row.name }),
    {},
  );
  const oldNames = current.map((r) => r.name).sort();
  const newNames = [...nextIds].map((id) => names[id] ?? id).sort();
  if (oldNames.join('|') !== newNames.join('|')) {
    logAudit({
      ...
      entityLabel: before.sku,
      action: 'UPDATE',
      changes: [{ field: 'collections', label: 'Collections', oldValue: oldNames, newValue: newNames }],
      userId,
    });
  }
  run('UPDATE product SET modified_by_id = ?, updated_at = ? WHERE id = ?', [userId, nowIso(), productId]);
...
export function displayStatus(status: ProductStatus): string {
...
export function diffFor(before: Record<string, unknown>, after: Record<string, unknown>) {
  return diffChanges(before, after, FIELD_LABELS);
}
