...
export function ImportPreview({
  ...
  return (
    <Card title="Validation">
      ...
      <form action={action} className="flex flex-wrap items-center gap-2 border-t border-ink-200 px-4 py-2.5">
        ...
        {state && (
          <span className={`text-xs ${state.ok ? 'text-emerald-700' : 'text-red-600'}`}>{state.ok ? state.message : state.error}</span>
        )}
      </form>
    </Card>
  );
